...
def create_email_view():
    ...
    email.set_task_id(str(async_task.task_id))

    db.session.add(email)
    db.session.commit()

    task_async_result = super_slow_email_sender.AsyncResult(str(async_task.task_id))
...
def retrieve_email_view(email_id):
    ...
    task_async_result = super_slow_email_sender.AsyncResult(str(email.task_id))
    status = task_async_result.status
    ...
    data = email_schema.dump(email).data
    return jsonify(data=data, status=status, result=result)
